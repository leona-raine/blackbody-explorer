...
def export_2d_csv(wavelengths, radiances_list, temps):
    import csv
    buf = io.StringIO()
    writer = csv.writer(buf)
    header = ["Wavelength (m)"] + [f"{T} K" for T in temps]
    writer.writerow(header)
    for i in range(len(wavelengths)):
        row = [wavelengths[i]] + [rad[i] for rad in radiances_list]
        writer.writerow(row)
...
col_vis_type, col_plot = st.columns([1, 3])
...
with col_vis_type:
    ...
    radiances_list = [planck_law(wavelengths, T) for T in example_temps]

    # 2D csv&png download
    st.download_button(
        label="📥 Download 2D CSV",
        data=export_2d_csv(wavelengths, radiances_list, example_temps),
        file_name="blackbody_2d.csv",
        mime="text/csv"
    )

    fig2d_for_png = plot_blackbody_2d(example_temps, wl_min_m, wl_max_m, n_points=n_wavelengths)
    st.download_button(
        label="📥 Download 2D PNG",
        data=export_2d_png(fig2d_for_png),
...
with col_plot:
    ...
    if st.session_state.view_mode == "surface":
        fig3d_static = plot_blackbody_3d_interactive(example_temps, [wl_min_m, wl_max_m])
        st.plotly_chart(fig3d_static, use_container_width=True)
    elif st.session_state.view_mode == "animated":
        fig3d_animated = plot_blackbody_3d_animated(
            wl_min_nm=wl_min_nm,
            wl_max_nm=wl_max_nm,
            temp_min=example_temps[0],
            temp_max=example_temps[-1],
        ...
        st.plotly_chart(fig3d_animated, use_container_width=True)
    else:
